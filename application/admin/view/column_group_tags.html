...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" method="post" action="{}">
		...
		<label><input type="checkbox" name="tags" value="{$vo.id}">{$vo.name}</label>
		{/volist}
...
$(function(){
	...
	$('#showlist').click(function(){
		var type = $('#type').val();
		var areaUrl = "{:url('admin/ColumnGroup/area')}";
		var tagsUrl = "{:url('admin/ColumnGroup/tags')}";
		if (type == 'area') {
			layer_window('列表',areaUrl);
		}else{
			layer_window('列表',tagsUrl);
		}
	});
	//表单验证
