...
class TagsGroupController extends BaseController
{
	...
	public function content(){
		...
		$me = $this->tagsGroup->with('tagss')->find($id);
		$contentList = $me['tagss'];
		
		return $this->fetch('',['me' => $me,'contentList' => $contentList]);
	}
	public function add(){
		$tagsList = $this->tags->all();
		return $this->fetch('',['tagsList' => $tagsList]);
	}
	...
	public function save(){
		...
		if (!empty($data['id'])) {
			return $this->update($data,$data['tags']);
		}
		//把$data提交Model层
		$result = $this->tagsGroup->add($data,$data['tags']);
		if ($result) {
	...
	public function update($data,$relation){
		...
		$res = $this->tagsGroup->put($data,$relation);
		if ($res) {
			$this->success('更新成功');
		}else{
